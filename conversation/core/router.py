...
class ConversationRouter:
    ...
    phases.md 2-6 handle_turn() 구현:
    1. short_buf 수집
    2. VDB 시맨틱 검색
    3. Context Assembly (PromptBuilder)
    4. LLM 생성
    5. mood / affection 상태 업데이트
    6. 세션에 턴 기록
    7. 요약 트리거 체크 → VDB 저장
    """
    ...
    def handle_turn(self, user_input: str, stream: bool = True) -> str:
        ...
        # 1. 단기 버퍼 (전체 로그 — Layer D에서 토큰 예산 기준으로 자름)
        ...
        # 3. Context Assembly
        messages = self.builder.assemble(short_buf=short_buf, vdb_results=vdb_results)
        messages.append({"role": "user", "content": user_input})

        # 4. LLM 생성
        response = self.llm.generate(messages, stream=stream)

        # 5. mood / affection 업데이트 (사용자 입력 기준)
