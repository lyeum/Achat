...
# ══════════════════════════════════════════════════════════════════════════════
# 5. 웹 검색 2-step — WebSearchTool.execute() 전체 플로우
#
#    흐름: execute({"query": ...})
#          → DDG 검색 (mock)
#          → Step1: 사실 추출 LLM (mock)
#          → Step2: 캐릭터 어조 변환 LLM (mock)
#          → 출처 목록 + 캐릭터 응답 합쳐서 반환
#
#    검증 포인트:
#      · 출처(제목, URL)가 최종 출력에 포함되는지
#      · 출처가 캐릭터 응답보다 앞에 위치하는지
#      · LLM 없을 때 raw 결과만 반환되는지
#      · Step1 실패 시 raw 폴백이 동작하는지
#      · Step2 실패 시 Step1 결과로 폴백하는지
# ══════════════════════════════════════════════════════════════════════════════

class TestWebSearchFullFlow:
    """
    전체 흐름:
    execute({"query": ...})
    → DDG 검색 (mock)
    → 유사도 스코어링 → 재정렬
    → HTML hyperlink 포맷 반환
    """
    ...
    _FAKE_DDG_RESULTS = [
        {
            ...
            "snippet": "파이썬은 1991년 귀도 반 로섬이 만든 언어다.",
        },
        {
            "title": "Python 위키백과",
            "url": "https://ko.wikipedia.org/wiki/파이썬",
            "snippet": "파이썬(Python)은 인터프리터 언어다.",
        },
    ]
    ...
    def _mock_ddg(self):
        return patch("tools.search.web_search._ddg_search", return_value=self._FAKE_DDG_RESULTS)

    # ── HTML hyperlink 형식 ───────────────────────────────────────────────────

    def test_output_contains_html_anchor_for_each_result(self):
        """각 결과 URL이 <a href> 태그로 감싸진다."""
        with self._mock_ddg():
            result = self._tool().execute({"query": "파이썬 역사"})
        assert 'href="https://docs.python.org"' in result
        assert 'href="https://ko.wikipedia.org' in result

    def test_output_contains_all_titles(self):
        """모든 검색 결과 제목이 포함된다."""
        with self._mock_ddg():
            result = self._tool().execute({"query": "파이썬 역사"})
        assert "Python 공식 문서" in result
        assert "Python 위키백과" in result

    def test_output_contains_snippets(self):
        """각 결과의 snippet이 출력에 포함된다."""
        with self._mock_ddg():
            result = self._tool().execute({"query": "파이썬 역사"})
        assert "귀도 반 로섬" in result
        assert "인터프리터" in result

    def test_result_count_in_header(self):
        """헤더에 결과 건수(2건)가 표시된다."""
        with self._mock_ddg():
            result = self._tool().execute({"query": "파이썬"})
        assert "2건" in result

    # ── 유사도 스코어링 ──────────────────────────────────────────────────────

    def test_higher_relevance_result_appears_first(self):
        """쿼리 토큰과 더 많이 겹치는 결과가 앞에 위치한다."""
        results = [
            {"title": "무관한 글", "url": "https://a.com", "snippet": "전혀 관계없는 내용"},
            {"title": "파이썬 역사", "url": "https://b.com", "snippet": "파이썬 역사는 1991년부터"},
        ]
        with patch("tools.search.web_search._ddg_search", return_value=results):
            result = self._tool().execute({"query": "파이썬 역사"})
    ...
    def test_empty_query_returns_error(self):
        result = self._tool().execute({"query": ""})
        assert "오류" in result

    def test_ddg_failure_returns_error(self):
        """DDG 검색 실패 → 오류 메시지 반환."""
        with patch("tools.search.web_search._ddg_search", side_effect=Exception("네트워크 오류")):
            result = self._tool().execute({"query": "파이썬"})
        assert "오류" in result

    def test_no_results_returns_message(self):
        with patch("tools.search.web_search._ddg_search", return_value=[]):
            result = self._tool().execute({"query": "xyzzy_없는검색"})
