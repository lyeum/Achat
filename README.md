...
3. "다음" 클릭 → 설치 진행 (의존성 자동 설치, 수 분 소요)

**2단계: 모델 파일 배치**

설치가 완료된 폴더(기본 `C:\Achat`) 안에 `models` 폴더가 있습니다.
`model_q4km.gguf` 파일(약 2 GB)을 그 안에 복사합니다.

```
C:\Achat\
└── models\
    └── model_q4km.gguf   ← 여기에 복사
```
...
2. `models\model_q4km.gguf` 파일을 압축 해제 폴더의 `models\` 안에 복사
